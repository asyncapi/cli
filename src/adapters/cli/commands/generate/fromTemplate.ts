...
export default class Template extends Command {
  ...
  async run() {
    ...
    const specification = await this.loadSpecification(asyncapi);
    ...
    try {
      ...
      await Template.generatorService.generate(specification.text(), template, output, options, genOption);
  ...
  private async runWatchMode(asyncapi: string | undefined, template: string, output: string, watchHandler: ReturnType<typeof this.watcherHandler>) {
    ...
    const transpiledTemplatePath = path.resolve(watchDir, GeneratorService.TRANSPILED_TEMPLATE_LOCATION);
    ...
    if (!await isLocalTemplate(path.resolve(GeneratorService.DEFAULT_TEMPLATES_DIR, templateName))) {
  ...
  private loadSpecification = async (asyncapi: string | undefined): Promise<Specification> => {
    try {
