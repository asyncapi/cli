...
import { ServiceResult, ValidationOptions, ValidationResult } from '@/interfaces';
...
export default class Validate extends Command {
  ...
  async run() {
    ...
    const result = await this.validationService.validateDocument(this.specFile, validateOptions);
  ...
  private async handleDiagnostics(result: ServiceResult<ValidationResult>, flags: any): Promise<void> {
    const diagnosticsFormat = flags['diagnostics-format'] ?? 'stylish';
    const hasIssues = (result.data?.diagnostics && result.data.diagnostics.length > 0) ?? false;
    ...
    if (flags.output) {
      const { success, error } = await this.validationService.saveDiagnosticsToFile(
        flags.output,
        diagnosticsFormat,
        diagnosticsOutput,
      );
      ...
      if (!success) {
        this.logToStderr(error || 'Failed to save diagnostics to file', { exit: 1 });
      } else {
