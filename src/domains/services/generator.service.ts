...
export class GeneratorService extends BaseService {
  ...
  async generate(asyncapi: Specification, template: string, output: string, options: GenerationOptions, genOption: any, interactive = this.defaultInteractive): Promise<ServiceResult<GenerationResult>> {
    ...
    try {
      await generator.generateFromString(asyncapi.text(), { ...genOption, path: asyncapi });
    } catch (err: any) {
      console.log(err);
  ...
  async generateUsingNewGenerator(asyncapi: Specification, template: string, output: string, options: any, genOption: any) : Promise<ServiceResult<GenerationResult>> {
    const v3NotSupported = this.checkV3NotSupported(asyncapi, template);
    if (v3NotSupported) {
      return this.createErrorResult(v3NotSupported);
    }
    ...
    logs.push(`${yellow('Check out your shiny new generated files at ') + magenta(output) + yellow('.')}\n`);
