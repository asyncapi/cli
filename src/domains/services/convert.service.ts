...
import { convert, convertOpenAPI, convertPostman, OpenAPIConvertVersion } from '@asyncapi/converter';
...
export class ConversionService extends BaseService {
  ...
  async convertDocument(
    ...
    case 'asyncapi': 
      convertedDocument = convert(specFile.text() ?? '', options['target-version'] || '3.0.0');
      break;
    case 'openapi':
      convertedDocument = convertOpenAPI(specFile.text() ?? '', specFile.toJson().openapi as OpenAPIConvertVersion, {
