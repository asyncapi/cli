  ...
  async validateDocument(
    ...
    try {
      ...
      if (suppressAllWarnings || suppressedWarnings.length) {
        activeParser = await this.buildAndRegisterCustomParser(specFile, suppressedWarnings, suppressAllWarnings);
      } else {
        activeParser = this.parser;
      }

      const { document, diagnostics } = await activeParser.parse(specFile.text(), {
        source: specFile.getSource(),
      });
