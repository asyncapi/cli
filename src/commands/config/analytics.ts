...
export default class Analytics extends Command {
  ...
  async run() {
    ...
    try {
      ...
      switch (e.code) {
      case 'ENOENT':
        this.error(`Unable to access the analytics configuration file. We tried to access the ${cyan('.asyncapi-analytics')} file in the path "${cyan(analyticsConfigFile)}" but the file could not be found.`);
        break;
      case 'EEXIST':
        this.error(`Unable to update the analytics configuration file. We tried to update your ".asyncapi-analytics" file in the path "${cyan(analyticsConfigFile)}" but the file does not exist.`);
        break;
      default:
