...
export const fromTemplateFlags = () => {
  return {
    ...
    'disable-hook': Flags.string({
      char: 'd',
      description:
        'Disable a specific hook type or hooks from a given hook type',
      multiple: true,
    }),
    ...
    'registry-url': Flags.string({
      ...
      description:
        'Specifies the URL of the private registry for fetching templates and dependencies',
    }),
    'registry-auth': Flags.string({
      description:
        'The registry username and password encoded with base64, formatted as username:password',
    }),
    'registry-token': Flags.string({
      description:
        'The npm registry authentication token, that can be passed instead of base64 encoded username and password',
    }),
