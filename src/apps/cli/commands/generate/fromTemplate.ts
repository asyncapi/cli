...
export default class Template extends Command {
  ...
  static args = {
    asyncapi: Args.string({
      description:
        '- Local path, url or context-name pointing to AsyncAPI file',
      required: false,
    }),
    template: Args.string({
      description:
        '- Name of the generator template like for example @asyncapi/html-template or https://github.com/asyncapi/html-template',
      required: false,
    }),
  };
  ...
  async run() {
    ...
    const parsedFlags = this.parseFlags(
      flags['disable-hook'],
      flags['param'],
      flags['map-base-url'],
      flags['registry.url'],
    ...
    if (flags['map-base-url']) {
      genOption.resolve = {
        resolve: this.getMapBaseUrlToFolderResolver(
          parsedFlags.mapBaseUrlToFolder,
        ),
      };
    }
    ...
    if (watchTemplate) {
      const watcherHandler = this.watcherHandler(
        asyncapi,
        template,
        output,
        options,
  ...
  private parseFlags(
    ...
    return {
      ...
      registryAuthentication: this.registryValidation(
        registryUrl,
        registryAuth,
        registryToken,
      ),
  ...
  private async registryValidation(
    registryUrl?: string,
    registryAuth?: string,
    registryToken?: string,
  ) {
    ...
    try {
      ...
      if (response.status === 401 && !registryAuth && !registryToken) {
        this.error(
          'You Need to pass either registryAuth in username:password encoded in Base64 or need to pass registryToken',
          { exit: 1 },
        );
      }
    } catch (error: any) {
  ...
  private async runWatchMode(
    ...
    template: string,
    output: string,
    watchHandler: ReturnType<typeof this.watcherHandler>,
  ) {
    ...
    const transpiledTemplatePath = path.resolve(
      watchDir,
      AsyncAPIGenerator.TRANSPILED_TEMPLATE_LOCATION,
    );
    ...
    if (specificationFile) {
      ...
      this.log(
        `[WATCHER] Watching for changes in the template directory ${magenta(watchDir)} and in the AsyncAPI file ${magenta(specificationFile)}`,
      );
      watcher = new Watcher([specificationFile, watchDir], ignorePaths);
    } else {
      this.log(
        `[WATCHER] Watching for changes in the template directory ${magenta(watchDir)}`,
      );
    ...
    if (
      !(await isLocalTemplate(
        path.resolve(AsyncAPIGenerator.DEFAULT_TEMPLATES_DIR, templateName),
      ))
    ) {
      this.warn(
    ...
    await watcher.watch(watchHandler, (paths: any) => {
      this.error(
        `[WATCHER] Could not find the file path ${paths}, are you sure it still exists? If it has been deleted or moved please rerun the generator.`,
        {
          exit: 1,
        },
      );
